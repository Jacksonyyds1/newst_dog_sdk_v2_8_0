...
// 	shell_print(sh, "V: %.3f, I: %.3f, T: %.2f, ", (double)info.voltage, (double)info.current, (double)info.temp);
// 	shell_print(sh, "State Of Charge: %.2f, Time Till Empty: %.2f, Time Till Full: %.2f\n",
// 		    (double)info.soc, (double)info.tte, (double)info.ttf);
...
void read_fuel_gauge_info(const struct shell *sh, size_t argc, char **argv)
{
    ...
    shell_print(sh, "Voltage:         %.3f V", (double)info.voltage);
    shell_print(sh, "Current:         %.3f mA", (double)info.current);
    shell_print(sh, "Temp:            %.2f C", (double)info.temp);

    shell_print(sh, "Charge Level   : %.2f%%", (double)info.soc);
    shell_print(sh, "Time Till Empty: %.2f s", (double)info.tte);
    shell_print(sh, "Time Till Full:  %.2f s", (double)info.ttf);
