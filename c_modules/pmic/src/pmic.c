...
void batt_info_work_handler(struct k_work *work)
{
    ...
    if (batt_info.temp <= 0) {
        ...
        if (!get_vbus_present()) {
            ...
            if (batt_info.temp < CONFIG_MIN_ALERT_BATTERY_TEMP / 10.0f) {
                curr_temp_range = TOO_COLD;
            } else if (batt_info.temp > CONFIG_MAX_ALERT_BATTERY_TEMP / 10.0f) {
        ...
        if ((batt_info.temp > max_batt_temp) || (batt_info.temp < min_batt_temp)) {
            if (uicr_shipping_flag_get()) {
                ...
                LOG_ERR("Battery Temp out of range: %f", (double)batt_info.temp);
        ...
        if (batt_info.voltage < min_v_for_9160) {
            if (uicr_shipping_flag_get()) {
                LOG_ERR("Battery Voltage too low for 9160, turning it off: %f", (double)batt_info.voltage);
        ...
        } else if (batt_info.voltage > (min_v_for_9160 + 0.1f)) {    // added some pad to prevent rapid on/off
            if (modem_power_state == PMIC_LTE_POWER_OFF) {
                LOG_INF("Battery Voltage is good for 9160, turning it on: %f", (double)batt_info.voltage);
                modem_power_on();
            }
        }
    }
...
int fuel_gauge_get_latest(fuel_gauge_info_t *latest)
{
    ...
    soc = nrf_fuel_gauge_process(voltage, current, temp, delta, false, NULL);
    soc = MAX(soc, 0);
    soc = MIN(soc, 100);
