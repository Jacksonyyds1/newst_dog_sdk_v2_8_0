    ...
    struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
    ...
    // Create zcbor state variable for decoding.
    ZCBOR_STATE_D(decoding_state, 3, buf, len, 6, 0);
...
static int device_info_resp(void)
{
    ...
    success = zcbor_tstr_put_term(encoding_state, status_str, strlen(status_str));
    if (!success) {
        goto error;
    }
    ...
    success = zcbor_tstr_put_term(encoding_state, uicr_serial_number_get(), strlen(uicr_serial_number_get()));
    if (!success) {
        goto error;
    }
...
static int battery_info_resp(void)
{
    ...
    LOG_DBG("fuel guage soc: %.2f", (double)fuel.soc);
    ...
    success = zcbor_tstr_put_term(encoding_state, charging_str, strlen(charging_str));
    if (!success) {
        goto error;
    }
...
static int modem_info_resp(void)
{
    ...
    // Add the ICCID value
    success = zcbor_tstr_put_term(encoding_state, iccid_str, strlen(iccid_str));
    if (!success) {
        goto error;
    }
    ...
    // Add the IMEI value
    success = zcbor_tstr_put_term(encoding_state, imei_str, strlen(imei_str));
    if (!success) {
        goto error;
    }
...
static int wifi_info_resp(void)
{
    ...
    success = zcbor_tstr_put_term(encoding_state, macaddr, strlen(macaddr));
    if (!success) {
        goto error;
    }
    ...
    success = zcbor_tstr_put_term(encoding_state, version, strlen(version));
    if (!success) {
        goto error;
    }
    ...
    success = zcbor_tstr_put_term(encoding_state, connection_status_str, strlen(connection_status_str));
    if (!success) {
        goto error;
    }
...
static int read_next_wifi_ap_info(uint16_t len, uint16_t offset)
{
    ...
    success = zcbor_tstr_put_term(encoding_state, ap_key, strlen(ap_key));
    if (!success) {
        goto error;
    }
    ...
    success = zcbor_tstr_put_term(encoding_state, wifi_ap->ssid, strlen(wifi_ap->ssid));
    if (!success) {
        goto error;
    }
    ...
    success = zcbor_tstr_put_term(encoding_state, wifi_ap->flags, strlen(wifi_ap->flags));
    if (!success) {
        goto error;
    }
...
static int charging_notify(char *status)
{
    if (notify_enable) {
        ...
        // Add the charging status value
        success = zcbor_tstr_put_term(encoding_state, status, strlen(status));
        if (!success) {
            goto error;
        }
...
static int wifi_connect_success_notify(char *ip_addr)
{
    if (notify_enable) {
        ...
        success = zcbor_tstr_put_term(encoding_state, ip_addr, strlen(ip_addr));
        if (!success) {
            goto error;
        }
    ...
    struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
    ...
    /* Create zcbor state variable for decoding. */
    ZCBOR_STATE_D(decoding_state, 3, buf, len, 4, 0);
